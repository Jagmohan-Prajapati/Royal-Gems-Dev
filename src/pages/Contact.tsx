...
export default function Contact() {
  const [formData, setFormData] = useState({ name: '', email: '', subject: 'Bespoke Consultation', message: '' });
    ...
    <div className="bg-surface min-h-screen text-on-surface select-none pb-0">
      ...
      <main className="pt-32 pb-20 px-4 md:px-gutter max-w-container-max mx-auto">
        ...
        <div className="text-center mb-16 select-text">
          <h1 className="font-display-hero text-[44px] md:text-display-hero text-primary mb-4 uppercase">
            Contact Our Atelier
          </h1>
          <p className="font-body-lg text-body-lg text-text-muted max-w-2xl mx-auto italic">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-20">
          ...
          {/* Left Column: Form */}
          <div className="lg:col-span-7 bg-surface-parchment p-8 md:p-10 border border-border-sepia shadow-sm">
            <h2 className="font-headline-md text-headline-md text-primary mb-8 border-b border-border-sepia/30 pb-4 uppercase tracking-wider">
              Inquiry Form
            </h2>

            {submitted ? (
              <div className="bg-white/60 border border-success-forest/30 p-8 text-center space-y-4">
                <CheckCircle className="h-12 w-12 text-success-forest mx-auto animate-bounce" />
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="font-label-caps text-label-caps text-primary uppercase font-bold text-[10px] tracking-wider block">
                      Name
                    </label>
                    <input
                      type="text"
                      ...
                      className="w-full bg-surface-bright border border-secondary/30 p-4 font-body-md text-body-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all rounded-none"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="font-label-caps text-label-caps text-primary uppercase font-bold text-[10px] tracking-wider block">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className="w-full bg-surface-bright border border-secondary/30 p-4 font-body-md text-body-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all appearance-none rounded-none"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows={6}
                    className="w-full bg-surface-bright border border-secondary/30 p-4 font-body-md text-body-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all resize-none rounded-none"
                  />
                </div>

                <button
                  type="submit"
                  className="w-full md:w-auto px-12 py-4 bg-primary text-on-primary font-label-caps text-label-caps uppercase tracking-widest border border-secondary hover:bg-primary-container transition-all duration-300 active:scale-98"
                >
                  Send Message
                </button>
              </form>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-dark-burgundy p-8 md:p-10 text-secondary-fixed shadow-xl">
              <h2 className="font-headline-sm text-headline-sm text-secondary-fixed mb-2 uppercase">
                Our Concierge
              </h2>
              <p className="font-body-sm text-body-sm text-surface-variant uppercase tracking-widest text-[11px] opacity-80 mb-6">
                Available Monday - Saturday
              </p>
              
              {/* Double Rule line */}
              <div className="h-1 border-t border-b border-border-sepia/20 w-full my-6" />

              <div className="space-y-8 select-text">
                <div className="flex items-start gap-4">
                  <Mail className="h-6 w-6 text-secondary-fixed-dim mt-1" />
                  <div>
                    <h4 className="font-label-caps text-label-caps uppercase text-secondary-fixed-dim mb-1 font-bold text-[10px] tracking-wider">
                      Electronic Mail
                    </h4>
                    <p className="font-body-lg text-body-md md:text-body-lg hover:underline">
                ...
                <div className="flex items-start gap-4">
                  <PhoneCall className="h-6 w-6 text-secondary-fixed-dim mt-1" />
                  <div>
                    <h4 className="font-label-caps text-label-caps uppercase text-secondary-fixed-dim mb-1 font-bold text-[10px] tracking-wider">
                      Telephone
                    </h4>
                ...
                <div className="flex items-start gap-4">
                  <MapPin className="h-6 w-6 text-secondary-fixed-dim mt-1" />
                  <div>
                    <h4 className="font-label-caps text-label-caps uppercase text-secondary-fixed-dim mb-1 font-bold text-[10px] tracking-wider">
                      Jaipur Flagship
                    </h4>
                    <p className="font-body-lg text-body-md md:text-body-lg leading-relaxed">
              ...
              <div className="mt-8">
                <img
                  ...
                  className="w-full h-48 object-cover border border-secondary/20"
            ...
            <div className="bg-surface-parchment p-6 border border-border-sepia">
              <div className="aspect-video w-full relative overflow-hidden border border-border-sepia/20">
                <img
              ...
              return (
                <div key={index} className="bg-surface-parchment border border-border-sepia">
                  <button
                    ...
                    className="flex justify-between items-center p-6 w-full text-left font-headline-sm text-[20px] text-on-surface"
                  >
                    <span>{faq.q}</span>
                    <ChevronDown className={`h-5 w-5 text-secondary transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {isOpen && (
                    <div className="px-6 pb-6 font-body-md text-body-md text-text-muted leading-relaxed animate-in fade-in slide-in-from-top-1 duration-200">
                      {faq.a}
                    </div>
                  )}
                </div>
